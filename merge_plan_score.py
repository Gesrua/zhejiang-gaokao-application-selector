...
df1 = pd.read_csv("2021模拟_普通类平行计划1_普通类平行录取_物理化学生物.csv")
df2 = pd.read_csv('2021模拟_普通类平行计划1_普通类平行录取_政治历史地理.csv')

d2020 = pd.read_csv("2020.csv", index_col='序号')
...
def addDf(df):
  for index, row in df.iterrows():
    if not is_unique(row):
      continue
    option = dict((key, row[key]) for key in ["院校名称","专业名称","省","城市","选考科目要求", "备注"])
    option["分数线"] = query_score(row['院校代码'], row['专业代码'])
